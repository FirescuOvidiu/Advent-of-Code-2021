...
int main()
{
  ...
  vector<bool> visited(boards.size(), false);
  bool         foundWinnedBoard = false;
  int          count            = 0;

  for (const auto & number : numbers)
  {
    for (int it = 0; it < boards.size(); it++)
    {
      if (visited[it])
        continue;

      for (auto & line : boards[it])
      {
      ...
      if (checkIfBoardWins(boards[it]))
      {
        visited[it] = true;
        count++;
        if (count == boards.size())
        {
          out << number * getUnmarkedNumbersSum(boards[it]);
